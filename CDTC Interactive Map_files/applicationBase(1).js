...
define(["require", "exports", "./support/configParser", "esri/core/promiseUtils", "esri/identity/IdentityManager", "esri/identity/OAuthInfo", "esri/portal/Portal", "esri/portal/PortalItem", "esri/portal/PortalQueryParams", "esri/config", "../structuralFunctionality/locale", "esri/intl", "./support/configParamsUtils"], function (require, exports, configParser_1, promiseUtils_1, IdentityManager_1, OAuthInfo_1, Portal_1, PortalItem_1, PortalQueryParams_1, config_1, locale_1, intl_1, configParamsUtils_1) {
    ...
    class ApplicationBase {
        ...
        //
        //--------------------------------------------------------------------------
        async queryGroupItems(groupId, itemParams, portal) {
            if (!portal || !groupId) {
                portal = this.portal;
            }
            ...
            const params = new PortalQueryParams_1.default(paramOptions);
            const result = await portal.queryItems(params);
            return result;
        }
        async load(template) {
            const { settings } = this;
            const { group: groupSettings, portal: portalSettings, webMap: webMapSettings, webScene: websceneSettings, urlParams: urlParamsSettings, } = settings;
            const isEsri = await this._isEnvironmentEsri();
            const urlParams = (0, configParser_1.parseConfig)(this._getUrlParamValues(urlParamsSettings));
            ...
            if (this?.config?.env) {
                ...
                this.config.proxyUrl = this._getEsriEnvironmentProxyUrl(this.config.portalUrl);
            }
            ...
            const checkAppAccess = IdentityManager_1.default.checkAppAccess(sharingUrl, oauthappid)
            ...
            return (0, promiseUtils_1.eachAlways)([
                ...
                const [applicationItemResponse, applicationDataResponse, portalResponse, checkAppAccessResponse,] = applicationArgs;
                ...
                const applicationConfig = (0, configParser_1.parseConfig)(applicationData ? applicationData.values : null);
                const portal = portalResponse ? portalResponse.value : null;
                this.portal = portal;
                // Detect IE 11 and older
                this.isIE = this._detectIE();
                this.locale = (0, locale_1.defineLocale)({ portal, config: this.config });
                this.direction = (0, intl_1.prefersRTL)(this.locale) ? "rtl" : "ltr";
                ...
                const configParamsJSON = (0, configParamsUtils_1.getConfigParams)(template);
                // GET DEFAULT VALES OBJECT DEFINED IN CONFIG PARAMS JSON
                const defaultValues = (0, configParamsUtils_1.generateDefaultValuesObj)(configParamsJSON);
                this.config = this._mixinAllConfigs({
                    config: this.config,
                    defaultValues,
                    url: urlParams,
                ...
                if (isWebMapEnabled) {
                    const maps = withinConfigExperience && draft?.webmap
                        ? [draft.webmap, webmap]
                        : webmap;
                ...
                if (isWebSceneEnabled) {
                    const scenes = withinConfigExperience && draft?.webscene
                ...
                if (isGroupInfoEnabled) {
                    const draftGroups = withinConfigExperience && draft?.group
                        ? [draft.group, group]
                        : group;
        ...
        addReqInterceptorsForProxies(config) {
        ...
        //
        //--------------------------------------------------------------------------
        _mixinSettingsDefaults(settings) {
        ...
        _limitItemSize(items, allowMultiple) {
            const firstItem = items[0];
            return allowMultiple ? items : firstItem ? [firstItem] : [];
        }
        _getPropertyArray(property) {
        ...
        async _queryGroupInfo(groupId, portal) {
            const params = new PortalQueryParams_1.default({
                query: `id:"${groupId}"`,
            });
            return (await portal.queryGroups(params));
        }
        _loadItem(id) {
            const item = new PortalItem_1.default({
                id,
            });
            return item.load();
        }
        _overwriteItemsExtent(responses, applicationItem) {
            if (!responses) {
                return;
            }
        ...
        _overwriteItemExtent(item, applicationItem) {
        ...
        _registerOauthInfos(appId, portalUrl, usePopupWorkflow) {
            if (!appId) {
                return;
            }
            const shouldUsePopup = usePopupWorkflow ||
        ...
        _urlToObject() {
            const query = (window.location.search || "?").substr(1), map = {};
            const urlRE = /([^&=]+)=?([^&]*)(?:&+|$)/g;
        ...
        _formatUrlParamValue(urlParamValue) {
        ...
        _isWithinConfigurationExperience() {
         ...
         */
        _renderLocalTestCasesUI(testCases, template) {
